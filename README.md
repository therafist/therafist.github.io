...
- **תפריט נגישות ייעודי** (כפתור צף): הגדלת/הקטנת טקסט, גווני אפור, ניגודיות גבוהה,
  הדגשת קישורים, עצירת אנימציות, פונט קריא ואיפוס — נשמר ב-localStorage.
- **עמוד הצהרת נגישות**: `accessibility.html` (מקושר מהפוטר ומתפריט הנגישות).
- HTML סמנטי, ניווט מקלדת מלא (כולל תפריט מובייל, לייטבוקס ותפריט הנגישות), `alt` לכל תמונה.
- קישור "דלג לתוכן המרכזי", ניגודיות גבוהה, תמיכה ב-`prefers-reduced-motion`.
